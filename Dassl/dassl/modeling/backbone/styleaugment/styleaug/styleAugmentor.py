...
class StyleAugmentor(nn.Module):
    def __init__(self):
        ...
        # load checkpoints:
        checkpoint_ghiasi = torch.load(join(dirname(__file__),'checkpoints/transformer.pth'))
        checkpoint_stylepredictor = torch.load(join(dirname(__file__),'checkpoints/stylepredictor.pth'))
        checkpoint_embeddings = torch.load(join(dirname(__file__),'checkpoints/embeddings.pth'))
