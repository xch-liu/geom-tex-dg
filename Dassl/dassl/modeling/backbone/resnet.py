...
class StyleAugmentor(nn.Module):
    def __init__(self, beta_mode=False, batch_ratio=0.5, alpha=0.5):
        ...
        self.alpha = alpha
        # load checkpoints:
        checkpoint_ghiasi = torch.load(join(dirname(__file__),'styleaugment/styleaug/checkpoints/transformer.pth'))
        checkpoint_stylepredictor = torch.load(join(dirname(__file__),'styleaugment/styleaug/checkpoints/stylepredictor.pth'))
        checkpoint_embeddings = torch.load(join(dirname(__file__),'styleaugment/styleaug/checkpoints/embeddings.pth'))
