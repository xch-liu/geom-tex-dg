...
AVAI_CHOICES = [
    ...
    'randomgrayscale', 'gaussian_blur', 'random_deformation'
]
...
class RandomDeformation:
    """Given an image of (height, width), we perform random deformation.
    ...
    def __call__(self, img):
        ...
        alpha = self.height * 2
        sigma = self.height * 0.08
        alpha_affine = self.height * 0.08
        random_state = None
...
def _build_transform_train(cfg, choices, expected_size, normalize):
    ...
    '''Randomo Geometric Deformation'''
    if 'random_deformation' in choices:
        print('+ random geometric deformation')
        tfm_train += [
            RandomDeformation(cfg.INPUT.SIZE[0], cfg.INPUT.SIZE[1], cfg.DEFORM_RATIO)
        ]
